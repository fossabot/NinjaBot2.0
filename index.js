...
if (message.content.toLowerCase().startsWith(config.prefix + `add`)) {
 ...
 if(command === "help") {
  message.delete().catch();	 
   ...
   .addField("Sendembed", "Send an embeded message ``nb/sendembed {Embeded Message Here}")
   .addField("suggest", "Sends your suggestion to the placeholder support server ``nb/suggest {Suggestion}")
   .addField("Removerole", "Removes the mentioned role from the mentioned user ``nb/removerole @user @role")
   .addField("Say", "Sends your provided message ``nb/say {Message Here}")
   .addField("Purge", "Deletes the provided number of messages **nb/purge <number of messages>**")
   .addField("ticketcreate", "Opens a new support ticket ``nb/ticketcreate {reason}")
